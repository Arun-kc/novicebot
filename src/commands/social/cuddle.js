...
module.exports = {
	...
	aliases : [],
	category: 'gif',
	description: 'Shows a random cuddling gif',
	async execute(client, msg, args) {

		const keywords = 'anime cuddle';
		...
		if(args.length < 1 || args[0] === 'me' || args[0] === `<@!${member.id}>`) {
			const embed = new Discord.MessageEmbed()
				.setTitle(`🤗 ${member.nickname || member.user.username} cuddles themselves`)
				.setImage(gif)
				.setColor('RANDOM')
				.setTimestamp();
			...
			const embed = new Discord.MessageEmbed()
				.setTitle(`🤗 ${member.nickname || member.user.username} cuddles ${usersMap} 😋`)
				.setImage(gif)
				.setColor('RANDOM')
				.setTimestamp();
			msg.channel.send(embed);
