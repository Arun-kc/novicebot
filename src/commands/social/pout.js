...
module.exports = {
	...
	aliases : ['pouts'],
	category: 'gif',
	description: 'Shows a random pouting gif',
	async execute(client, msg, args) {

		const keywords = 'anime pout';
		...
		if(args.length < 1 || args[0] === 'me' || args[0] === `<@!${member.id}>`) {
			const embed = new Discord.MessageEmbed()
				.setTitle(`😤 ${member.nickname || member.user.username} pouts at themselves`)
				.setImage(gif)
				.setColor('RANDOM')
				.setTimestamp();
			...
			const embed = new Discord.MessageEmbed()
				.setTitle(`😤 ${member.nickname || member.user.username} pouts at ${usersMap} 😤`)
				.setImage(gif)
				.setColor('RANDOM')
				.setTimestamp();
			msg.channel.send(embed);
