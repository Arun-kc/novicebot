...
module.exports = {
	...
	aliases : ['pokes'],
	category: 'gif',
	description: 'Shows a random poking gif',
	async execute(client, msg, args) {

		const keywords = 'anime poke';
		...
		if(args.length < 1 || args[0] === 'me' || args[0] === `<@!${member.id}>`) {
			const embed = new Discord.MessageEmbed()
				.setTitle(`👉 ${member.nickname || member.user.username} pokes themselves`)
				.setImage(gif)
				.setColor('RANDOM')
				.setTimestamp();
			...
			const embed = new Discord.MessageEmbed()
				.setTitle(`👉 ${member.nickname || member.user.username} pokes ${usersMap} 😋`)
				.setImage(gif)
				.setColor('RANDOM')
				.setTimestamp();
			msg.channel.send(embed);
