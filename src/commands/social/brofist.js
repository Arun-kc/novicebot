...
module.exports = {
	name : 'brofist',
	aliases : ['bf', 'fistbump', 'fb'],
	category: 'gif',
	description: 'Shows a random fist bump gif',
	async execute(client, msg, args) {

		const keywords = 'fist bump';
		...
		if(args.length < 1 || args[0] === 'me' || args[0] === `<@!${member.id}>`) {
			...
			const embed = new Discord.MessageEmbed()
				.setTitle(`🤜 ${member.nickname || member.user.username} gives fist bump to ${usersMap} 🤛`)
				.setImage(gif)
				.setColor('RANDOM')
				.setTimestamp();
			msg.channel.send(embed);
