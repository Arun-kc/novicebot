...
module.exports = {
	...
	aliases : [],
	category: 'gif',
	description: 'Shows a random biting gif',
	async execute(client, msg, args) {

		const keywords = 'anime bite';
		...
		if(args.length < 1 || args[0] === 'me' || args[0] === `<@!${member.id}>`) {
			const embed = new Discord.MessageEmbed()
				.setTitle(`😋 ${msg.member.nickname || member.user.username} bites themselves`)
				.setImage(gif)
				.setColor('RANDOM')
				.setTimestamp();
			...
			const embed = new Discord.MessageEmbed()
				.setTitle(`🧛 ${msg.member.nickname || member.user.username} bites ${usersMap} 😋`)
				.setImage(gif)
				.setColor('RANDOM')
				.setTimestamp();
			msg.channel.send(embed);
