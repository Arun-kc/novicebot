...
module.exports = {
	...
	aliases : ['pats'],
	category: 'gif',
	description: 'Shows a random pat gif',
	async execute(client, msg, args) {

		const keywords = 'anime pat';
		...
		if(args.length < 1 || args[0] === 'me' || args[0] === `<@!${member.id}>`) {
			const embed = new Discord.MessageEmbed()
				.setTitle(`😋 ${member.nickname || member.user.username} pats themselves`)
				.setImage(gif)
				.setColor('RANDOM')
				.setTimestamp();
			...
			const embed = new Discord.MessageEmbed()
				.setTitle(`😊 ${member.nickname || member.user.username} pats ${usersMap} 😋`)
				.setImage(gif)
				.setColor('RANDOM')
				.setTimestamp();
			msg.channel.send(embed);
